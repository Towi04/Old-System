...
@section('titulo')
    Reporte de alumnos programados 
@endsection
...
@section('breadcrumb')
    <ol class="breadcrumb no_print">
        ...
        <li class="breadcrumb-item active">
            <strong>Reporte de alumnos programados</strong>
        </li>
    </ol>
@endsection
...
@section('contenido')
    ...
    <div class="row ">
        <div class="col-lg-12 no_print">
            <div class="btn-group ">
                {{-- <a href="{{ route('reportes.programados',['tipo' => 'dia']) }}&@if(isset($_GET['id_asesor']))id_asesor={{$_GET['id_asesor']}}@endif"
                    class="btn @if ($tipo=='dia' ) btn-primary  text-white @else btn-white @endif">Día</a> --}}
                <a href="{{ route('reportes.programados',['tipo' => 'semanal']) }}&@if(isset($_GET['id_asesor']))id_asesor={{$_GET['id_asesor']}}@endif"
                    class="btn  @if ($tipo=='semanal' ) btn-primary  text-white @else btn-white @endif">Semanal</a>
                {{-- <a href="{{ route('reportes.programados',['tipo' => 'mes']) }}&@if(isset($_GET['id_asesor']))id_asesor={{$_GET['id_asesor']}}@endif"
                    class="btn  @if ($tipo=='mes' ) btn-primary  text-white @else btn-white @endif">Mensual</a>
                <a href="{{ route('reportes.programados',['tipo' => 'anual']) }}&@if(isset($_GET['id_asesor']))id_asesor={{$_GET['id_asesor']}}@endif"
                    ...
                    <div class="row">
                        ...
                        @if (request('tipo','semanal') == 'dia')
                            <div class="col-1 col-xxl-1 col-xl-1 col-md-1 col-sm-1">
                                <a href="{{ route('reportes.programados',['tipo' => 'dia','fecha'=>$fecha_antes->format('d-m-Y') ]) }}"
                            ...
                            <div class="col-1 col-xxl-1 col-xl-1 col-md-1 col-sm-1">
                                <a href="{{ route('reportes.programados',['tipo'=>'dia','fecha' => $fecha_despues->format('d-m-Y') ]) }}"
                        ...
                        @if (request('tipo','semanal') == 'mes')
                            <div class="col-1 col-xxl-1 col-xl-1 col-md-1 col-sm-1">
                                <a href="{{ route('reportes.programados',['tipo'=>'mes','fecha'=> $fecha_antes->format('d-m-Y') ]) }}"
                                    data-toggle="tooltip"
                            ...
                            <div class="col-1 col-xxl-1 col-xl-1 col-md-1 col-sm-1">
                                <a href="{{ route('reportes.programados',['tipo' => 'mes','fecha' => $fecha_despues->format('d-m-Y') ]) }}"
                                    data-toggle="tooltip" data-title="{{ $fecha_despues->format('d \d\e F \d\e\l Y') }}"
                                    data-placement="top" class="btn btn-primary btn-sm text-white no_print"><i
                                        class="fas fa-arrow-alt-circle-right fa-2x"></i></a>
                            </div>
                        ...
                        @if (request('tipo','semanal') == 'semanal')
                            <div class="col-1 col-xxl-1 col-xl-1 col-md-1 col-sm-1">
                                <a href="{{ route('reportes.programados',['tipo' => 'semanal','fecha' => $fecha_antes->startOfWeek()->format('d-m-Y') ]) }}"
                                    data-toggle="tooltip"
                                    data-title="{{ $fecha_antes->format('d \d\e F  \d\e\l  Y') }}"
                                    data-placement="top"
                                    class="btn btn-primary btn-sm text-white no_print">
                            ...
                            <div class="col-1 col-xxl-1 col-xl-1 col-md-1 col-sm-1">
                                <a href="{{ route('reportes.programados',['tipo'=> 'semanal','fecha'=>$fecha_despues->startOfWeek()->format('d-m-Y')]) }}"
                                    data-toggle="tooltip" data-title="{{ $fecha_despues->format('d \d\e F \d\e\l Y') }}"
                                    data-placement="top" class="btn btn-primary btn-sm text-white no_print"><i
                                        class="fas fa-arrow-alt-circle-right fa-2x"></i></a>
                            </div>
                        ...
                        @if (request('tipo','semanal') == 'anual')
                            <div class="col-1 col-xxl-1 col-xl-1 col-md-1 col-sm-1">
                                <a href="{{ route('reportes.programados',['tipo' => 'anual','fecha'=> $fecha_antes->format('d-m-Y'), ]) }}"
                                    data-toggle="tooltip"
                            ...
                            <div class="col-1 col-xxl-1 col-xl-1 col-md-1 col-sm-1">
                                <a href="{{ route('reportes.programados',['tipo'=>'anual','fecha'=> $fecha_despues->format('d-m-Y') ]) }}"
                                                ...
                                                @foreach ($programados as $programado)
                                                    <tr >
                                                        <td class="text-primary text-center">
                                                                <a target="_blank" href="{{route('alumnos.show', $programado->id_alumno)}}">{{ $programado->alumno->numero_control_fullname }}</a>
                                                        </td>
                                                        {{-- ESPECIALIDAD --}}
                                                        <td>
                                                            {{ optional($programado->especialidad)->nombre }} 
                                                        </td>
                                                        {{-- GRUPO --}}
                                                        <td>
                                                            @if($programado->alumno->inscripciones->where('id_especialidad',$programado->id_especialidad)->first())
                                                            {{ optional(optional($programado->alumno->inscripciones->where('id_especialidad',$programado->id_especialidad)->first())->grupo)->clave }}
                                                             @else 
                                                             {{ optional(optional($programado->alumno->inscripciones->filter(function($model)use($programado){
                                                                return $model->grupo->especialidad->id == $programado->id_especialidad;
                                                             })->first())->grupo)->clave }}
                                                             @endif
                                                        </td>
                                                        <td>
                                                            {{ optional($programado->fecha_inicio)->format('d-m-Y') }} 
                                                        </td>

                                                        <td>
                                                            @if($programado->alumno->inscripciones->where('id_especialidad',$programado->id_especialidad)->first())
                                                            {{ optional(optional($programado->alumno->inscripciones->where('id_especialidad',$programado->id_especialidad)->first())->fecha)->format('d-m-Y') }}
                                                             @else 
                                                             {{ optional(optional($programado->alumno->inscripciones->filter(function($model)use($programado){
                                                                return $model->grupo->especialidad->id == $programado->id_especialidad;
        ...
        <div class="col-lg-2 mb-3 no_print">
            ...
            <div class="col-sm-12 col-xxxl-12 p-1">
                <a class="element-box el-tablo p-3 text-center" href="#">
                  ...
                  <div class="value" >
                    <span id="span_abonos_monto">{{ number_format($programados->count(), 0, '.', ',') }}</span>
                  </div>
                </a>
            </div>
    ...
    <script type="text/javascript">
        $(function(){
            ...
            $('#datepicker').on('changeDate', function() {
                window.location = "{{route('reportes.programados')}}?tipo={{$tipo}}&fecha="+$('#datepicker').datepicker('getFormattedDate')+"&forma_pago={{@$_GET['forma_pago']}}"
            });
